<template>
	<x-form padding :saving="isSaving" @save="save" @cancel="cancel" v-if="show">
		...
		<x-row line>
			...
			<x-form-group col="8" :label="$t('organization')" :errors="errors.organization_id" optional>
				<x-typeahead-table removable :value="form.organization" url="organizations" name="name"
					:columns="['number', 'name']"
