<template>
	...
	<x-panel padding margin>
		...
		<x-row line>
			<x-form-group col="8" :label="$t('default_vendor_category_id')" :errors="errors.default_vendor_category_id">
				<x-tag-input :value="form.default_vendor_category" resource="vendor_categories" column="name" name="name"
				    @input="value => { form.default_vendor_category_id = value.id; form.default_vendor_category = value }">
				</x-tag-input>
			</x-form-group>
		</x-row>
	...
	<mini-crud url="settings/vendor_categories"
