<template>
	<div class="content-inner" v-if="show">
		<x-panel padding margin>
			...
			<x-row line>
				<x-form-group col="8" :label="$t('proposal_status_on_create_id')" :errors="errors.proposal_status_on_create_id">
					<x-tag-input :value="form.proposal_status_on_create" resource="proposal_statuses" column="name" name="name"
					    @input="value => { form.proposal_status_on_create_id = value.id; form.proposal_status_on_create = value }">
					</x-tag-input>
				</x-form-group>
				<x-form-group col="8" :label="$t('proposal_status_on_email_sent_id')" :errors="errors.proposal_status_on_email_sent_id">
					<x-tag-input :value="form.proposal_status_on_email_sent" resource="proposal_statuses" column="name" name="name"
					    @input="value => { form.proposal_status_on_email_sent_id = value.id; form.proposal_status_on_email_sent = value }">
					</x-tag-input>
				</x-form-group>
				<x-form-group col="8" :label="$t('default_proposal_template_id')" :errors="errors.default_proposal_template_id">
					<x-tag-input :value="form.default_proposal_template" resource="templates" column="name" name="name" :params="{type_id: '1'}"
					    @input="value => { form.default_proposal_template_id = value.id; form.default_proposal_template = value }">
					</x-tag-input>
				</x-form-group>
			</x-row>
			<x-row line>
				<x-form-group col="8" type="number" :label="$t('close_proposal_after_days')" :errors="errors.close_proposal_after_days" v-model="form.close_proposal_after_days"></x-form-group>
			</x-row>
			<x-row line>
				<x-form-group col="24" :label="$t('proposal_email_subject')"
					:errors="errors.proposal_email_subject"
					v-model="form.proposal_email_subject">
					</x-form-group>
				<x-form-group col="24" :errors="errors.proposal_email_template" :label="$t('proposal_email_template')">
		...
		<mini-status-crud url="settings/proposal_statuses"
			title="proposal_statuses">
			<x-tr slot="heading">
