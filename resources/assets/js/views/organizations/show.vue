<template>
	<div class="content-inner" v-if="show">
		<x-panel padding margin>
			<div slot="title">
				<router-link to="/organizations">{{$t('organization')}}</router-link> / {{model.name}}
				<small>({{model.number}})</small>
			</div>
			<div slot="extra">
				<router-link :to="`/organizations`" class="btn btn-default btn-sm">
					<small class="icon icon-arrow-left-c"></small>
				</router-link>
				<router-link :to="`/organizations/${model.id}/edit`" class="btn btn-default btn-sm">
					<small class="icon icon-edit"></small>
				</router-link>
				<x-button size="sm" type="error" icon="trash-b" @click="removeDB('organizations', model.id)"></x-button>
			</div>
				<x-row line>
		...
		<x-mini :url="`contacts?organization_id=${this.model.id}`" title="contacts">
			<div slot="extra">
		    <router-link :to="`/contacts/create?organization_id=${model.id}`" class="btn btn-sm btn-success">
			...
			<x-tr slot="heading">
				...
				<x-td type="th" size="6">{{$t('email')}}</x-td>
