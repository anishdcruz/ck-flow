<template>
	<x-form padding :saving="isSaving" @save="save" @cancel="cancel" v-if="show">
		...
		<x-row line>
			...
			<x-form-group col="8" :label="$t('category')" :errors="errors.organization_category_id">
				<x-tag-input :value="form.category" resource="organization_categories" column="name" name="name"
				    @input="value => { form.organization_category_id = value.id; form.category = value }">
				</x-tag-input>
			</x-form-group>
			<x-form-group col="8" v-model="form.name" :label="$t('name')" :errors="errors.name"></x-form-group>
		</x-row>
		<x-row line>
			<x-form-group col="8" v-model="form.email" :label="$t('email')" :errors="errors.email" optional></x-form-group>
